...
now and is cited on the line. The four documentation components, held together in
[`documentation.md`](documentation.md), are ordinary work orders and carry no such line.
...
Every ledger row names its spec or says why it has none: **23 of 23**. That is a coverage
...
| **The social preview card** | none: it is one command reading registered figures, and a work order would say less than this row | 2026-09-04 | [`benchmarks/social.py`](../../benchmarks/social.py), output gitignored | The card GitHub shows wherever this repository is linked, drawn rather than typed: the two means and the segment they are means over come through the figure registry ([`D-158`](../decisions.md#d-158)) and the grids come from `benchmarks.figure` whole, so a reworded table stops the command instead of producing a stale card. **The committed drawing could not be reused as it is**, and the reasons are worth keeping: it is 504 by 486 against a card's 2:1, its labels are 9.5px, and its greys are tuned for a documentation page read at full size, so the card darkens them rather than changing the figure. Its panel captions come off too, because 6 against 2 beside 12.36 against 2.40 is two number pairs for one idea. The image is gitignored for the reason the landing page left: it is uploaded in the repository's settings and nothing here links to it |
...
A spec is merged when its component is one of several that ran as one piece of work, never
when its findings became inconvenient. Every finding travels with it, and the ledger row
stays: it points at the section instead of at a file.

| Was | Where it went |
| --- | --- |
| `documentation-restructure.md` | [`documentation.md`](documentation.md#the-restructure) |
| `decision-curation.md` | [`documentation.md`](documentation.md#curating-the-decision-records) |
| `spec-reconstruction.md` | [`documentation.md`](documentation.md#specs-for-the-built-components) |
| `doc-citations.md` | [`documentation.md`](documentation.md#citations-in-source) |

Merged on 2026-09-03 ([`D-157`](../decisions.md#d-157)). Four components over two days, each
depending on the one before it, and three of the four fully spent: what a reader still needs
from them is four durable findings and a redirect table, not four work orders.
