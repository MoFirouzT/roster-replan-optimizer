...
*Assumes: the rules a returned roster is checked against, [`rules.md`](rules.md); the tenant profile a request names, [`configuring.md`](configuring.md).*
...
*Why async, why the wire schema is its own schema, and why the recommendation list is grouped rather than ranked: [`decisions.md`](../decisions.md#by-theme), under* Service, runtime and the fallback ladder *and* Explaining an answer.
