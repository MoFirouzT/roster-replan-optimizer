...
| [`presolve.md`](presolve.md) | Eliminating impossible (employee, shift) pairs before the solver | **28% off build, 16% off search, 24/24 cases** — a quarter of the model removed. Not "the largest single win" — that is `Instance.window` (`D-092`). [`D-045`] |
